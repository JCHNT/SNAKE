...
export const Game = () => {
  ...
  const renderGame = () => (
    <div className="flex flex-col items-center gap-4">
      <div className="flex justify-between w-full px-4 py-2 bg-gray-100 rounded-lg">
        <div>Score: {score}</div>
        <div>Meilleur: {highScore}</div>
        <div>Niveau: {difficulty}</div>
      </div>
  ...
  const renderHighScores = () => (
    <div className="flex flex-col gap-4 w-full max-w-md">
      <h2 className="text-2xl font-bold text-center">Meilleurs scores</h2>
      <div className="bg-gray-100 p-4 rounded-lg">
        <p className="text-xl text-center">Record actuel: {highScore}</p>
      </div>
      <button
        onClick={() => setGameState('menu')}
        className="w-full bg-gray-500 text-white px-4 py-2 rounded-lg hover:bg-gray-600 transition-colors flex items-center gap-2 justify-center"
